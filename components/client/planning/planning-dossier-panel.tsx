...
export function PlanningDossierPanel({ dossier, isLoading }: PlanningDossierPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-zinc-700 bg-zinc-950/70 p-4">
      <div className="mb-3">
        <div className="text-sm text-zinc-400">Hồ sơ</div>
        <div className="text-base font-semibold text-zinc-100">{dossier.tenHoSo}</div>
        <div className="text-xs text-zinc-500">Mã: {dossier.maHoSo}</div>
      </div>

      {!docs.length ? (
        <div className="text-sm text-zinc-400">Không có tài liệu đính kèm.</div>
      ) : (
        <div className="space-y-2">
          {docs.map((doc) => (
            <div
              key={doc.id}
              className="flex items-start justify-between gap-3 rounded-lg border border-zinc-700 bg-zinc-900/60 p-3"
            >
              ...
              {doc.sourceUrl ? (
                <a
                  ...
                  className="inline-flex items-center gap-1 rounded-md border border-sky-500/40 bg-sky-500/10 px-2 py-1 text-xs font-medium text-sky-200 hover:bg-sky-500/20"
                >
                  <LinkIcon className="h-3.5 w-3.5" />
                  Mở PDF
                </a>
              ) : (
                <span className="rounded-md border border-zinc-600 px-2 py-1 text-xs text-zinc-400">
                  Không có link
                </span>
              )}
            </div>
