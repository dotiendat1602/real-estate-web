...
export function PropertyImagesFieldAdmin({
  label = "Hình ảnh",
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-5">
        ...
        {value.length === 0 ? (
          <div className="mt-4 flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 py-10">
            <ImageIcon className="h-10 w-10 text-gray-400" />
            <div className="text-center">
              <div className="text-gray-700 font-semibold">Chưa có ảnh</div>
          ...
          <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
            {value.map((img) => (
              <div
                key={img.id}
                className={cn(
                  "group relative overflow-hidden rounded-lg border border-gray-200 bg-white hover:border-blue-300 transition-colors"
                )}
              >
                ...
                <div className="absolute left-2 top-2">
                  {img.isPrimary ? (
                    <span className="inline-flex items-center gap-1 rounded-md bg-blue-600 px-2 py-1 text-xs font-semibold text-white shadow-sm">
                    ...
                    <button
                      ...
                      className="rounded-md bg-black/60 px-2 py-1 text-xs text-white hover:bg-black/80 shadow-sm"
                    >
                      Đặt ảnh chính
                    </button>
                  )}
                ...
                <div className="absolute inset-x-0 bottom-0 flex items-center justify-between gap-2 p-2 bg-gradient-to-t from-black/70 to-transparent opacity-100 md:opacity-0 md:group-hover:opacity-100 transition">
                  <Button
                    ...
                    className="h-8 border-white/30 bg-white/90 text-gray-700 hover:bg-white text-xs"
                  >
                  ...
                  <Button
                    ...
                    className="h-8 border-white/30 bg-white/90 text-red-600 hover:bg-white hover:text-red-700 text-xs"
                  >
        ...
        {value.length > 0 && (
          <p className="mt-3 text-xs text-gray-500">
            Tip: Ảnh đầu tiên sẽ tự động được đặt làm Ảnh chính nếu chưa chọn.
          </p>
        )}
      </div>
    </section>
